...
describe('Message model tests', function () {
  ...
  it('Should be able to update an already existing record', function (done) {
    Message.where({ id: 1 }).fetch()
      .then(function (result) {
        expect(result.get('id')).to.equal(1);
      })
      .then(function () {
        return Message.where({ id: 1 }).save({ text: 'Bye Felicia' }, { method: 'update' });
      })
      .then(function () {
        return Message.where({ id: 1 }).fetch();
      })
      .then(function (result) {
        expect(result.get('text')).to.equal('Bye Felicia');
        done();
      })
      .catch(function (err) {
        // If this expect statement is reached, there's an error.
